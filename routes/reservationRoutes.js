    ...
    asyncHandler(async (req, res) => {
     ...
     FROM ( SELECT * from reservations WHERE email = ${JSON.stringify(email)} ) reservations INNER JOIN workshops ON reservations.workshop_id = workshops.id INNER JOIN events ON events.id = workshops.event_id;`)
            .then(([workshops, fieldData]) => {
                ...
                arrayIsEmpty(workshops)?res.status(404).json({ code: 404, msg: "No reservations found with this email" }) :res.status(200).json({ code: 200, msg: "ok", data:{reservation:reservationObject,event:eventObject,workshop:workshopObject} });
